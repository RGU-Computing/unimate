...
import { Card, CardElement, CardProps, StyleService, Text, useStyleSheet, Button, IconElement, Divider } from '@ui-kitten/components';
...
export const ActionCard = (props: ActionCardProps): CardElement => {
  ...
  if (data && data !== 'empty') ACTION = new Action(data.id, UtilService.getDateFromDatabaseDateFormat(data.data().date), data.data().text);
  
  useEffect(() => {
    
    if (data && data!== 'empty') {
      console.log('in')
      const subscriber = FirebaseService.subscribeForActionCardReacts(ACTION.id, onSuccess)
  ...
  const onLike = () => {
    if (_alreadyReacted()) {
      if (isLiked) {
        ...
        FirebaseService.updateActionCardReaction(ACTION.id, reaction, ACTION_CARDS.REACTS.LIKE)
      }
    } else {
      FirebaseService.addActionCardReaction(ACTION.id, ACTION_CARDS.REACTS.LIKE)
    }
  }
  ...
  const onHeart = () => {
    if (_alreadyReacted()) {
      if (isHearted) {
        ...
        FirebaseService.updateActionCardReaction(ACTION.id, reaction, ACTION_CARDS.REACTS.HEART)
      }
    } else {
      FirebaseService.addActionCardReaction(ACTION.id, ACTION_CARDS.REACTS.HEART)
    }
  }
  ...
  if (!data) {
    return (
      <ActivityIndicator size="large" color="#712177" style={{marginVertical: 20}} />
    )
  }

  if (data === 'empty') {
    return (
      <Text style={{textAlign: 'center', marginTop: 40}}>No Action Cards found for today! 😕</Text>
    )
  }
