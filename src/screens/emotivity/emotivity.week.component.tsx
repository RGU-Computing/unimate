...
export const EmotivityWeekScreen = ({ navigation }): React.ReactElement => {
    ...
    useEffect(() => {
        FirebaseService.getEmotivityData(UtilService.getDateWeekAgo(DATE.FORMATS.DB_UNIX), UtilService.getDateYesterday(DATE.FORMATS.DB_UNIX), onSuccessMoodTracking, onSuccessDiaryEntry);
    }, []);
    ...
    return (
        <ScrollView style={{flex: 1, alignSelf: 'center', backgroundColor: 'white', paddingHorizontal: 10}}>
            <Text style={{fontWeight: 'bold', fontSize: 20, marginTop: 20, textAlign: 'center'}}>{UtilService.getDateWeekAgo() + ' to ' + UtilService.getDateYesterday()}</Text>
