...
import { Text, Divider, RangeCalendar, Button } from '@ui-kitten/components';
...
export const EmotivityCustomScreen = ({ navigation }): React.ReactElement => {
    ...
    useEffect(() => {
        if (range.endDate)
        FirebaseService.getEmotivityData(range.startDate.getTime(), range.endDate.getTime(), onSuccessMoodTracking, onSuccessDiaryEntry);
    }, [range]);
  ...
  return (
    <ScrollView style={{flex: 1, alignSelf: 'center', backgroundColor: 'white', paddingHorizontal: 10}}>
        {!range.endDate && 
        <>
            <Text style={{fontWeight: 'bold', fontSize: 20, marginVertical: 20, textAlign: 'center'}}>
        ...
        {range.endDate && 
        <>
            <Text style={{fontWeight: 'bold', fontSize: 20, marginTop: 20, textAlign: 'center'}}>
                {UtilService.getDateFromDatabaseDateFormat(range.startDate)} to {UtilService.getDateFromDatabaseDateFormat(range.endDate)}
            </Text>
            <Button style={{marginHorizontal: '20%', marginTop: 10}} onPress={() => setRange({startDate: null, endDate: null})}>Reset Date Range</Button>
            ...
            <ProgressChart
                data={{
                    ...
                    data: weeklyEmotivityData && weeklyEmotivityData !== 'empty' ? [weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.ANGER]/5, weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.ANXIETY]/5, weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.SADNESS]/5, weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.STRESS]/5, weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.TIRED]/5, weeklyEmotivityData[EMOTIVITY.DATABASE.FIELDS.HAPPINESS]/5] : [0, 0, 0, 0, 0, 0]
                }}
                width={screenWidth}
