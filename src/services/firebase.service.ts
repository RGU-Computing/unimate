...
import { ACTION_CARDS, DATE, EMOTIVITY, DIARY, USERS } from './types';
...
export class FirebaseService {
    ...
    static setPushToken(token) {
        ...
        firestore().collection(USERS.DATABASE.REF).doc(uid).update({
            pushToken: token
        })
        .then(function() {
    ...
    static getTodayActionCard = (onSuccess, onError = _onError) => {
        firestore().collection(ACTION_CARDS.DATABASE.REF).where(ACTION_CARDS.DATABASE.FIELDS.DATE, '==', UtilService.getDateToday(DATE.FORMATS.DB)).get().then(onSuccess, onError);
    }

    static subscribeForActionCardReacts = (docID, onSuccess, onError = _onError) => {
        return firestore().collection(ACTION_CARDS.DATABASE.REF).doc(docID).onSnapshot(onSuccess, onError)
    }
    ...
    static addActionCardReaction = (actionCardID, REACTION_TYPE) => {
        ...
        firestore().collection(ACTION_CARDS.DATABASE.REF).doc(actionCardID).update({
    ...
    static updateActionCardReaction = (actionCardID, oldReaction, REACTION_TYPE) => {
    ...
    static getIsEmotivityDoneToday = (onSuccess, onError = _onError) => {
        ...
        query = query.where(EMOTIVITY.DATABASE.FIELDS.DATE, '==', UtilService.getDateToday(DATE.FORMATS.DB_UNIX))
    ...
    static subscribeForEmotivity = (onSuccess, onError = _onError) => {
        ...
        let query = firestore().collection(EMOTIVITY.DATABASE.REF)
        query = query.where(EMOTIVITY.DATABASE.FIELDS.USER, '==', uid)
        return query = query.where(EMOTIVITY.DATABASE.FIELDS.DATE, '==', UtilService.getDateToday(DATE.FORMATS.DB_UNIX)).onSnapshot(onSuccess, onError);

    }

    static getEmotivityData = (startDate, endDate, onSuccessMood, onSuccessDiary, onError = _onError) => {

        const { uid } = AppStorage.getUser();
        firestore().collection(EMOTIVITY.DATABASE.REF).where(EMOTIVITY.DATABASE.FIELDS.USER, '==', uid).where(EMOTIVITY.DATABASE.FIELDS.DATE, '>=', startDate).where(EMOTIVITY.DATABASE.FIELDS.DATE, '<=', endDate).get().then(onSuccessMood, onError);
        firestore().collection(DIARY.DATABASE.REF).where(DIARY.DATABASE.FIELDS.USER, '==', uid).where(EMOTIVITY.DATABASE.FIELDS.DATE, '>=', startDate).where(EMOTIVITY.DATABASE.FIELDS.DATE, '<=', endDate).get().then(onSuccessDiary, onError);
    ...
    static addMoodTrackingRecord = (scores) => {
        ...
        const { uid } = AppStorage.getUser();
        firestore().collection(EMOTIVITY.DATABASE.REF).add({
            [EMOTIVITY.DATABASE.FIELDS.USER]: uid,
            [EMOTIVITY.DATABASE.FIELDS.DATE]: UtilService.getDateToday(DATE.FORMATS.DB_UNIX),
    ...
    static addNewDiaryEntry = (STATUS_TYPE, data) => {
        ...
        firestore().collection(DIARY.DATABASE.REF).add({
            [DIARY.DATABASE.FIELDS.USER]: uid,
            [DIARY.DATABASE.FIELDS.CONVERSATIONS]: data,
            [DIARY.DATABASE.FIELDS.STATUS]: STATUS_TYPE,
            [DIARY.DATABASE.FIELDS.DATE]: UtilService.getDateToday(DATE.FORMATS.DB_UNIX),
        });
    ...
    static addReflection = (id, question) => {
        ...
        firestore().collection(DIARY.DATABASE.REF).doc(id).update({
            [DIARY.DATABASE.FIELDS.STATUS]: DIARY.DATABASE.STATUS.COMPLETE,
            [DIARY.DATABASE.FIELDS.CONVERSATIONS]: firebase.firestore.FieldValue.arrayUnion(question)
        }).catch(e => {
            console.warn(e)
        });
    ...
    static getTodayDiaryEntry = (onSuccess, onError = _onError) => {
        ...
        query = query.where(DIARY.DATABASE.FIELDS.DATE, '==', UtilService.getDateToday(DATE.FORMATS.DB_UNIX))
        query.get().then(onSuccess, onError);

    }

    static subscribeForDiaryEntry = (docID, onSuccess, onError = _onError) => {
        return firestore().collection(DIARY.DATABASE.REF).doc(docID).onSnapshot(onSuccess, onError)
    }

    static setUser = (user) => {

        const ref = firestore().collection(USERS.DATABASE.REF).doc(user.uid);
        firestore().runTransaction(async transaction => {
            const doc = await transaction.get(ref);
            if(!doc.exists) {
                transaction.set(ref, {user: user, dailyStepGoal: 5000, notifications: [
                    new Notification(
                        'Welcome to Unimate!',
    ...
    static getStepsToday = (goal, onSuccess) => {
        ...
        GoogleFit.authorize(options).then(res => {
            ...
            getSteps({ startDate: start, endDate: end }, null, res => {
                console.log(goal)
                AppStorage.setTraxivityDetails(goal, res.length > 0 ? res[0].value : 0);
                onSuccess()
            });
    ...
    static subscribeForTraxivity = (onSuccess, onError = _onError) => {
        ...
        const { uid } = AppStorage.getUser();
        firestore().collection(USERS.DATABASE.REF).doc(uid).onSnapshot(onSuccess, onError);
    ...
    static subscribeForNotifications = (onSuccess, onError = _onError) => {
        ...
        const { uid } = AppStorage.getUser();
        return firestore().collection(USERS.DATABASE.REF).doc(uid).onSnapshot(onSuccess, onError);
